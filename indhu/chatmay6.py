...
def complete_task( db, field_names, coll_name, prompt, chat_engine, mode, chat_store, input,context):
    ...
    task_details = {
        "templateName": additional_info.get('templateName'),
        "bot_level_addinfo": additional_info.get('additionalInfo'),
        "ruleId": additional_info.get('ruleId'),
        "personalInfo": personal_info_details.get('temp'),
        "startDate": template['temp'].get('startDate'),
        "assignToList": template['temp'].get('assignToList'),
        "dueDate": template['temp'].get('dueDate'),
        "templateId": template['temp'].get('templateId'),
        "eventStatusId": template['temp'].get('_id'),
        "eventId": template['temp'].get('eventId'),
        "variableList": task_level_template['temp'].get('variableList'),
        "additionalInfoVar": task_level_template['temp'].get('additionalInfoVar'),
        "taskLevelAddInfoVariables": task_level_template_all_values
    }
        ...
        if '$match' in stage:
            ...
            for key, value in stage['$match'].items():
                ...
                if isinstance(value, dict) and "$regex" in value:
                    print(value)
                    # Normalize regex pattern by removing spaces and lowercasing
                    raw_pattern = value["$regex"]
                    normalized_pattern = raw_pattern.replace(" ", "").lower()
                    options = value.get("$options", "")
                    if '$expr' not in transformed['$match']:
                        transformed['$match']["$expr"]={'$and': []}
                    
                    transformed['$match']["$expr"]['$and'].append({
