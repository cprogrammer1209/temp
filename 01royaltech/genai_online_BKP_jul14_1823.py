...
def make_api_call_with_retry(content,model="gemini-2.0-flash", max_retries=5, initial_delay=5):
    ...
    # GOOGLE_API_KEY='AIzaSyD-Gsdh5u9JamamQdzH-pIi-5q78GOxWV4'
    genaiclient = genai.Client(api_key=GOOGLE_API_KEY)
    # model.generate_content(query,generation_config={"temperature":0})
    generation_config = {
        "temperature": 0.0
    }
...
def fieldDetection(data,timestamp):
            ...
            if table_obj:
                ...
                if checkTable is False:
                    ...
                    if getChunkFromLLM:
                        ...
                        ### Instructions:
                        1. The document may contain multiple invoices. Your primary objective is to:
                        - Detect the **start and end page range of each Invoice Document**.
                        - Return the page ranges **separately for each invoice**.
                        
                        2. Ignore any pages that belong to non-invoice documents (e.g., Shipping Documents, Packing Lists, Delivery Notes, etc.). Only **Invoice Documents** should be included.

                        3. For each Invoice Document:
                        - Identify the full page range (start to end pages) corresponding to that invoice.
                        - Assign a **invoice_slno** number based on its **sequential order** of appearance in the document (starting from 1).
                        - Example:
                            - First invoice → `invoice_slno: 1`
                            - Second invoice → `invoice_slno: 2`, and so on.
                        - This numbering should be based on the order of appearance in the OCR result, **not the actual invoice number mentioned in the document**.

                        4. **Chunking logic:**
                        - If a single invoice document spans more than 10 pages, **split it into multiple chunks**, each containing **up to 10 pages**.
                        - Chunks should **not split table rows** — if a table row spans multiple pages, keep those pages in the same chunk.
                        - **Do not merge multiple invoices into a single chunk** — each chunk must belong to only one invoice.

                        ### Summary of Rules:
                        - Extract only **Invoice Documents** page ranges.
                        - Identify start and end page range for each invoice.
                        - Number invoices with `invoice_slno` based on order of appearance.
                        - If any invoice exceeds 10 pages, split it into multiple chunks while respecting table row continuity.
                        - Each chunk should belong to only one invoice.
                        Input:
                        {output}
    ...
    if isTemplateBased:
        ...
        print(type(templateData))
        del templateData['_id']
        del templateData['input']
        ...
        for table in multi_table:        
            ...
            if "Item Details" in table:
                ...
                if 'ItemDetails' in templateData:
                    ...
                    ItemDetails['multiTable']="multiTable"
