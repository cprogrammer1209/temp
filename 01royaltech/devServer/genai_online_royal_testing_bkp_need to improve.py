...
def process_single_page_range(itr, page_output, prompt_format, table_obj, isTemplateBased, templateData, index):
    ...
    if isTemplateBased:
        ...
        templatePrompt = f'''
            ...
            ******The document provided is chunked set of invoices. Actual value of InvoiceSlNo: {itr['invoice_slno']}******
        ...
        # print("table chat format ", table_chat_format)
    ...
    print("table response ", table_content)
...
def merge_results_in_order(results, table_result, isTemplateBased):
...
def process_page_ranges_parallel(page_ranges, page_output, prompt_format, table_obj, 
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        future_to_index = {
            executor.submit(
                ...
                isTemplateBased, templateData, index
    ...
    total_input_tokens, total_output_tokens = merge_results_in_order(results, table_result, isTemplateBased)
...
def fieldDetection(data,timestamp):
    ...
    if (document_type_data):
        for fields in document_type_data['fields']:
            ...
            elif fields['isTable'] and fields['tableName'] and 'prompts' in fields and (fields['tableName'] == 'Invoice Details'  or fields['tableName'] == 'ShipmentContainerDetails' ) :
                if fields['tableName'] in headerPlusTable:
                    headerPlusTable[fields['tableName']][0][fields['fieldName']] = fields['prompts']
                else:
                    headerPlusTable[fields['tableName']] = [{}]
                    headerPlusTable[fields['tableName']][0][fields['fieldName']] = fields['prompts']
            elif fields['isTable'] and fields['tableName'] and 'prompts' in fields and not (fields['tableName'] == 'Invoice Details'  or fields['tableName'] == 'ShipmentContainerDetails' ):
    ...
    file_path = '/home/ubuntu/new_ssd/aiqod-staging-job/aiqod-stage-python/test/data/'+filename
    ...
    if model_name == 'gemini':
        ...
        if document_obj:
            ...
            if  isTemplateBased:
                ...
                headerTemplate=[]
                if 'inputHeader' in templateData[0]:
                    headerTemplate = templateData[0]['inputHeader']
                examples = ""
                for idx, item in enumerate(headerTemplate):
                    examples += f'''
                        Example Sample {idx}(Input → Output):
                        ***Input*** : {item['input']}
                        ***Output*** : {item['header']}
                ...
                print("headerPlusTable ",headerPlusTable)
                templatePrompt = f'''
                You are a highly accurate and efficient document data extraction assistant.
                ...
                You must extract the information only based on the fields listed below for header and table.

                *******Header Extraction Fields*******:
                {json.dumps(document_obj, indent=2)}

                *******Table Extraction Fields *******(need to populate all the fields. If any field value is missing or unavailable in the document, leave it as an empty string (""). ):
                {json.dumps(headerPlusTable, indent=2)}

                *******Guidelines*******:
                - For the header, populate only the fields listed under **Header Extraction Fields**.
                - For the table, **populate all fields as per the Table Extraction Fields (headerPlusTable)**. 
                - If any field is missing or unavailable in the document, leave its value as an empty string ("").
                - Do not skip any fields. Even if the document doesn’t contain certain fields, they must still appear in the final output with an empty string value.
            ...
            # print('invoice_obj before',invoice_obj)
            if 'header' in invoice_obj:
                temp = invoice_obj
                invoice_obj = {}
                invoice_obj = temp['header']

                invoice_obj['invoiceItems']= []
                invoice_obj['multiTable'] = []
                # print("checkign ",invoice_obj)
                if 'table' in temp and temp['table']:
                    # for key, value in temp['table'].items():
                    #     invoice_obj[key] = value
                    invoice_obj['multiTable'].append(temp['table'])
            ...
            if table_obj:
                ...
                if checkTable is False:
                    ...
                    input_token, output_token = process_page_ranges_parallel(
    ...
    from datetime import datetime
    token_details['createdAt'] = datetime.now()
    token_details['updatedAt'] = datetime.now()
    try:
