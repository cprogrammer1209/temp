...
# Middleware function to deduct credits
def limitcheck_middleware(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        print("-------------- GEN AI PREDICTION MIDDLEWARE ---------------")
        print("------------- DOCUMENT TYPE",documentType,"------------------------")
        if documentType == 'Invoice_Document':
            authorization = request.headers.get('authorization')
            print("authorization",authorization)
            orgId = request.headers.get('selectedorgid')
            print("Selected org id",orgId)
            if authorization: 
                res = decode_token(authorization)
                print("decoded_token",res)
                if not "error" in res:
                    ...
                    myclient = pymongo.MongoClient(config['dbConnection'])
                    db = myclient[config['db']]
                    subscriber_info = db.get_collection('subscribers').find_one({'_id': ObjectId(subscriber_id)})
                    creditBased = True if subscriber_info.get('creditBased') else False
                    info = {
                        "userId":userId,
                    ...
                    if creditBased:
                        res = deduct_credit(subscriber_info,17.5,db,info)
...
def genai_prediction():
    ...
    try:
        ...
        f_ = re.sub('b\'|\\\\n\'', '', str(subprocess.check_output(
            "/home/ubuntu/.conda/envs/python38/bin/python genai_online.py -i "+ "file_event_"+timestamp+".json" + " -t " + timestamp, shell=True , universal_newlines=True)))
...
def genai_prediction_royal_tech():
    # sem.acquire()
    print('using this api mlPrediction')
    try:
        data=request.json
...
def decode_token(token):
    ...
    if token and token.startswith("Bearer "):
        ...
        try:
            decoded_token = jwt.decode(token,config['jwtSecret'],"HS256")
            return decoded_token
        except ExpiredSignatureError:
            return {"error": "Token has expired"}
        except InvalidTokenError:
            return {"error": "Invalid token"}
    return {"error": "No valid Bearer token provided"}


def deduct_credit(sub_data,credit,db,info):
    print("info",info)
    features_array = sub_data['features'] if isinstance(sub_data.get('features'), list) else []
    extra_purchase = sub_data['extraPurchase'] if isinstance(sub_data.get('extraPurchase'), list) else []
    past_packages = sub_data['pastPackages'] if isinstance(sub_data.get('pastPackages'), list) else []
    packages = sub_data['packages'] if isinstance(sub_data.get('packages'), list) else []
    from datetime import datetime, timedelta
    current_date = datetime.now()
    features_arr = []
    exP = False
    for features in features_array:
        if 'expiry' in features and current_date >= features['expiry']:
            past_packages.append(features)

            check_ei = next((i for i, a in enumerate(extra_purchase) if str(a.get('packageId')) == str(features.get('packageId'))), -1)
            check_e = extra_purchase[check_ei] if check_ei != -1 else None

            if check_e:  # Check if extraPurchase with the same packageId exists
                features_list = copy.deepcopy(check_e)
                features_list['startData'] = current_date.strftime("%Y-%m-%d")
                features_list['expiry'] = add_days(current_date, features_list['expiry']).strftime("%Y-%m-%d")
                features_list['packageId'] = ObjectId(features['packageId'])
                features_arr.append(features_list)

                extra_purchase.pop(check_ei)
                exP = True
            else:
                packages = [a for a in packages if str(a) != str(features['packageId'])]
        else:
            features_arr.append(features)

    credits = []
    for fea in features_arr:
        if 'credits' in fea:
            credits.append(fea['credits'])
    used = sum(a.get('used', 0) for a in credits)
    limit = sum(a.get('limit', 0) for a in credits)
    if (limit >= used + credit):
        features_arr[0]['credits']['used'] = used + credit
        db.get_collection("credit_usages").insert_one({
                "orgId": ObjectId(info['orgId']),
                "subscriberId": ObjectId(info['subscriberId']),
                "userId": ObjectId(info['userId']),
                "deptId": ObjectId(info['deptId']) if info['deptId'] else info['deptId'],
                "activityType": "Invoice Extraction",
                "createdAt": current_date,
                "creditsUsed": credit
        })
        if(exP):
            db.get_collection('subscribers').update_one({'_id': sub_data['_id']},{
                "$set":{
                    "features": features_arr,
                    "packages": packages,
                    "pastPackages": past_packages,
