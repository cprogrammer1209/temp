...
def NudegGeneration():
    ...
    try:
        ...
        def generate_and_store_nudges(user_id_str, db_conn, llm_model):
            ...
            additional_info = ''
            try:
                employee_sales_details = db.get_collection('Wockhardt_Employee_Details').find_one({'userId':user_object_id})
                if employee_sales_details and 'Description' in employee_sales_details:
                    additional_info = employee_sales_details['Description']
            except Exception as e:
                print (e)
        ...
        {f'Additional information about the user: {additional_info}' if additional_info else ''}
...
def NudegGenerationCategoryWise():
    ...
    try:
        ...
        category_to_insert_in_db = input['category']

        import random
        if PREDEFINED_CATEGORIES == 'Sales Strategy':
            my_list = ['Sales Strategy','Sales','Product Selling Strategy']
            PREDEFINED_CATEGORIES = random.choice(my_list)
        elif PREDEFINED_CATEGORIES == 'Remainder/Follow-up Nudges':
            my_list = ['Remainder/Follow-up Nudges','Doctor Visit']
            PREDEFINED_CATEGORIES = random.choice(my_list)
        print("PREDEFINED_CATEGORIES ",PREDEFINED_CATEGORIES)
        def format_json_llm(text):
            """
            Extracts and repairs JSON from a text string, typically LLM output.
            """
        ...
        def generate_and_store_nudges(user_id_str, db_conn, llm_model, category):
            """
            Fetches data, generates nudges using an LLM, and stores them in MongoDB.
            """
            try:
            ...
            try:
                ...
                pipeline = [{'$match': {'userId': {'$in': [user_object_id]}}}]
                if 'Competitor Strategy Analysis' not in category:
                    pipeline.append( {'$project': {"compQuantity": 0, "compRxn": 0, "compValue": 0}})
                pipeline.append({'$sample': {'size': SAMPLE_SIZE}})
                rcpa_docs = list(db_conn.get_collection('RCPA').aggregate(pipeline))
                reporting_docs = list(db_conn.get_collection('reporting').aggregate([
                    {'$match': {'userId': {'$in': [user_object_id]}}},
                    {'$sample': {'size': SAMPLE_SIZE}}
                ]))
        ...
        {f'Additional information about the user: {additional_info}' if additional_info and 'Performance' in PREDEFINED_CATEGORIES else ''}
        ...
        *   **Category: Reminders & Admin Tasks**
        ...
        Respond **only** with the recommendation text, no other headers or explanations.
        Be creative.
        It is very crutial that only one recommendation per given category need to be created.
        Populate the output in the following JSON format:
        ```json
        {{
        "output": 
            {{
            {PREDEFINED_CATEGORIES}: ["recommendation1_for_category"]
            }}
        ...
        """
            ...
            # The LLM is expected to return a list containing one dictionary of categories.
            # Example: {"output": [{"Sales": [...], "Doctor Visit": [...]}]}
            print('final_output_json',final_output_json['output'].items())
            nudgeResponse = []
            for category_name, recommendations in final_output_json['output'].items():
                ...
                for recommendation_text in recommendations:
                    ...
                    # }
                    try:
                        previousNudge = nudges_collection.find_one({
                            "userId": user_object_id,
                            "category": category_to_insert_in_db,
                            "isDeleted": False,})
                        print('previousNudge',previousNudge)
                        if not previousNudge:
                            nudge_doc = {
                                "userId": ObjectId(user_id),
                                "response": [f'{recommendation_text}\nGenerated on {formatted_time}'],
                                "category": category_to_insert_in_db, # The actual category name from LLM
                                "isDeleted": False,
                                "createdAt": datetime.utcnow() # Good practice to have a creation timestamp
                            }
                            nudges_collection.insert_one(nudge_doc)
                        ...
                        if not isinstance(previousNudge.get("response"), list):
                            previousNudge["response"] = []
                        
                        previousNudge["response"] = [f'{recommendation_text}\nGenerated on {formatted_time}']+ previousNudge["response"][:4]

                        nudges_collection.update_one(
                            {"_id": previousNudge["_id"]},
                            {"$set": {"response": previousNudge["response"]}}
                        )
                        nudgeResponse= previousNudge["response"]
                        nudges_inserted_count += 1
                    except Exception as e:
                        print(f"Error inserting nudge into MongoDB: {e}. ")

            print(f"Successfully inserted {nudges_inserted_count} nudges for user {user_id}.")
            return nudgeResponse

        final_output_json = generate_and_store_nudges(user_id,db,model,PREDEFINED_CATEGORIES)
