...
def typeDetection(context,options,jsonFormat,input_token=0,output_token=0):
    
   
    try:
        user_prompt_for_type=f"""You are a helpful, smart, kind, and efficient QA assistant. You always fulfill the user's requests to the best of your ability.
        Given the following details of a document: content of the document is provided here : {context} . 
        from the given text. search for which document type it might belong to basend on content of the document. also search for sub_document type:
        Type and Subtype are two variables its data is provided here {options} .It is highly important to use only the Type and Subtype provided here. dont create new ones.
        also tell in one sentence me how did you come to conclusion that this document belongs to this Subtype category. give its response in comment variable. 
         Output should be in the form of this json.{jsonFormat}."""
        print('user_prompt_for_type',user_prompt_for_type)
        header_chat_format = [
            {"role": "user", "content": user_prompt_for_type}
        ]
        ...
        output_text = json.loads(response_content)
        if 'Type' and 'Subtype' in output_text:
            initial_type = output_text['Type']
            subtype = output_text['Subtype']
            print ('TYpe  result : ',initial_type, ' subtype result ',subtype)
            fields=[]
            if subtype in config['bgv_doc_type']['options']:
                fields = config['bgv_doc_type'][subtype]['options'] #bgv_doc_type 

            return initial_type,subtype,fields
        else:
...
def fieldDetection(data,timestamp):
    ...
    invoice_obj['orgId'] = ObjectId(data['input']['orgId'])
