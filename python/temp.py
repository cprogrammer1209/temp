...
Its very critical, If the question is not related to schema respond empty."""
                ...
                if input['previousCollection'] == 'workhardt':
                    ...
                    In Projection, all the field value must be in string format.so dont give it in object format. unwind it and then assign the field value in projection like above.
                    If lookup is not ued dont do these things.
                    It is very critical to Always use doctor name and employee name in the projection.
                    means all the date fields are already in date field only, so dont use for exisitng date field.
                    use when the date is need to use from llm like the user may ask for a spefic range and any particular range, in that time.
                ...
                final_query =  convert_date_fields(query)
