...
def fieldDetectorTawazun():
    ...
    with semaphore:
        result=fieldDetectionTawazunApi(data,timestamp)
...
def mlPrediction():
    ...
    try:
        ...
        if templateId!=None:
            data['input']['mlInput']['templateId'] = templateId
        ...
        f_ = re.sub('b\'|\\\\n\'', '', str(subprocess.check_output(
            "/usr/bin/python tawazun_prediction_api.py -i "+ "file_event_"+timestamp+".json" + " -t " + timestamp, shell=True , universal_newlines=True)))
...
@app.route('/gibots-pyapi/GenaimlPrediction',methods=['GET', 'POST'])
def GenaimlPrediction():
    # sem.acquire()
    print('using this api GenaimlPrediction')
    try:
        data=request.json
        ...
        #f_ = re.sub('b\'|\\\\n\'', '', str(subprocess.check_output(
        #"/home/anil/anaconda3/envs/kie-ocr/bin/python gen_ai_13b.py -i "+ "file_event_"+timestamp+".json" + " -t " + timestamp, shell=True)))
        from acheck_new import fieldDetection
        #llm = app.config['llm']
        #global_service_context = app.config['service_context']
        #print ('befor',get_gpu_memory_usage())
        outputData = fieldDetection(data)
        print(outputData)
        #print ('after',get_gpu_memory_usage())
        ...
        os.system("rm "+os.getcwd()+"/file_event_"+timestamp+".json")
        '''
        #result = fieldDetection(combineLines, documentType, ai_model_name, ref_no, filePath)
        # sem.release()
        #gc.collect()
        # import torch, gc
        # gc.collect()
...
def save_base64_as_png(base64_string, save_directory="uploaded_files"):
    ...
    try:
        ...
        file_path = os.path.join(save_directory, f"image_{timestamp}.png")
        ...
        raise Exception(f"Error saving PNG: {str(e)}")
"""
import random
import os
import string
...
def pdf_to_png(pdfpath,save_path):
    temp_path = save_path + ''.join(random.choices(string.ascii_uppercase + string.digits, k = 25))
    if not os.path.exists(temp_path):
        os.makedirs(temp_path)

    pdf_file_name = os.path.basename(pdfpath)
    filename, file_extension = os.path.splitext(pdf_file_name)

    pngpath = temp_path + "/" + filename + ".png"
    print(filename, file_extension)
    if file_extension == ".pdf" or file_extension == ".PDF":
        command = "convert -density 300 -background white -alpha remove " + pdfpath + " -verbose " + pngpath
        os.system(command)

    images_list = sorted(glob.glob(temp_path + "/*.png"))
    if len(images_list) > 9:
        images_list.sort(key=alphanum_key)

    images_array = []
    for i in range(0,len(images_list)):
        src = images_list[i]
        dst = save_path
        imagepath = dst + os.path.basename(src)
        images_array.append(imagepath)
        print(imagepath)
        copyfile(src, '%s/%s' % (dst, src.split('/')[-1]))
...
# Function to save base64 as PNG, JPG, or PDF
def save_base64_as_png_or_pdf(base64_string, save_directory="uploaded_files"):
    """
    Convert a base64 string to image(s) or PDF, handle conversion, and save to disk.

    Parameters:
    base64_string (str): The base64 encoded string of an image (.png, .jpg, .jpeg) or PDF file.
    save_directory (str): The directory where files will be saved.

    Returns:
    list: A list of dictionaries with file paths.
    """
    try:
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"document_{timestamp}.{file_extension}"
        ...
        if file_extension == "pdf":
            png_paths = pdf_to_png(file_path, save_directory)
            os.remove(file_path)  # Remove the original PDF to save space
            return [{"imageFilePath": path} for path in png_paths]
        else:
            return [{"imageFilePath": file_path}]

    except Exception as e:
        raise Exception(f"Error saving file: {str(e)}")

from werkzeug.utils import secure_filename
@app.route('/gibots-pyapi/Stamp_verification',methods=['GET', 'POST'])
def Stamp_verification():
    print('using this api StampValidation')
    ...
    try:
        ...
        timestamp = str(time.time())
        base64_string = data.get('Inputdocument')
        upload_folder = "/home/user/save-data-here/public-folder/demo-files/"
        file_path = save_base64_as_png_or_pdf(base64_string,upload_folder)
        print(f"File saved at: {file_path}")
               

        attest_country = data.get('AttestCountry', 'EGY') 
        stamp_validation = data.get('StampValidation', 'Yes')  
        #sign_validation = data.get('SignValidation', 'Yes') 
        text_extraction_required = data.get('TextExtractionRequired', 'Yes')  
        text_extraction_format = data.get('TextExtractionFormat', 'XN00000000123')  

        # Create the structure
        output_data = {
            "Inputdocument": file_path,
            "AttestCountry": attest_country,
            "StampValidation": stamp_validation,
            "TextExtractionRequired": text_extraction_required,
            "TextExtractionFormat": text_extraction_format
        }
        
        print("final_input_------",output_data)
        ...
        f_ = re.sub('b\'|\\\\n\'', '', str(subprocess.check_output(
            "/home/user/anaconda3/envs/custom_genai/bin/python mofa_final_api.py -i "+ "file_event_"+timestamp+".json" + " -t " + timestamp, shell=True)))

        print(f_)

        outputData = json.load(open("file_"+timestamp+".json", "r"))
        processing_time = time.time()- float(timestamp)
        outputData['ResponseTime'] = f"{processing_time:.2f} sec"
        #os.system("rm "+os.getcwd()+"/file_"+timestamp+".json")
        #os.system("rm "+os.getcwd()+"/file_event_"+timestamp+".json")
        ...
        #gc.collect()
        torch.cuda.empty_cache()
        print('ERROR: error in Stamp verification !!!',traceback.format_exc())
        print(e)
        outputData={'error_output':str(e), 'statusCode': '202'}
