...
function getUniqueRoutes(routes) {
  ...
  for (const route of routes) {
    ...
    const key = `${from}___${to}___${time}`; // <-- include time
...
function timeDecimalToFormattedString(decimal) {
  return DateTime.fromObject({ hour: 0 })
    .plus({ minutes: Math.floor(decimal * 1440) })
    .toFormat("hh:mm a");
}

function findMatchingSchedule(subTimeStr, dbRecords, fromCity, toCity) {
  for (const doc of dbRecords) {
    const dbTimeStr = timeDecimalToFormattedString(doc["Time"]);
...
function convertExcel(jsonData) {
  ...
  for (const mainRoute in jsonData) {
    const subroutes = jsonData[mainRoute];
  ...
  return {
    filepath,
    publicUrl,
    jsonData: rows, // Optional: flattened output
  };
}
...
app.post("/scrapper/playwright", async (req, res) => {
  try {
    ...
    const mongoDocs = await db
      .collection("prasannaPurpleBuses_")
      .find({})
      .toArray();
    uniqueRoutes = getUniqueRoutes(mongoDocs);

    console.log(uniqueRoutes);

    const { orgId, subscriberId, userId } = req.body;
    ...
    for (const route of uniqueRoutes) {
      ...
      route.paytmBusSource = route["From City"];
      route.paytmBusDestination = route["To City"];
      route.abhiBusSource = route["From City"];
      route.abhiBusDestination = route["To City"];
      ...
      function extractFromToFromScheduleCode(scheduleCode) {
        ...
        const match = rest.match(/^(.+?)\s+To\s+(.+?)(?:\s+-|\d|AM|PM|$)/i);
        if (!match) return { from: "Unknown", to: "Unknown" };
      ...
      route.paytmBusSource = from;
      route.paytmBusDestination = to;
      route.abhiBusSource = from;
      route.abhiBusDestination = to;
    ...
    let excelOutput = convertExcel(response);
    let excelFilePath = excelOutput["publicUrl"];
...
async function searchPaytm(route, response, mongoDocs) {
  ...
  try {
    ...
    for (let i = 0; i < buses.length; i++) {
      ...
      const mainDepartureTime = match.mainTime;
...
async function searchAbhiBus(route, response, mongoDocs) {
  ...
  try {
    ...
    for (const bus of buses) {
      ...
      let match = findMatchingSchedule(
        subArrivalTime,
        uniqueRoutes,
        route["From City"],
        route["To City"]
      );
      let mainRouteName, mainDepartureTime;
      ...
      let rawMainRouteName = match.scheduleCode;
      mainRouteName = extractMainRouteName(rawMainRouteName);

      mainDepartureTime = match.mainTime;

      let subrouteKey = `${route["From City"]} - ${route["To City"]}`;
      if (
        response[mainRouteName] &&
        response[mainRouteName][subrouteKey] &&
        response[mainRouteName][subrouteKey][subArrivalTime]
      ) {
        // Append abhibus rating to existing object
        response[mainRouteName][subrouteKey][subArrivalTime]["abhibus"] =
          rating;
      } else {
        if (!response[mainRouteName]) response[mainRouteName] = {};
        if (!response[mainRouteName][subrouteKey])
...
// function restructureByMainRoute(response) {
//   const finalOutput = {};

//   for (const routeKey in response) {
//     const records = response[routeKey]["n/a"];
//     const [mainFrom, mainTo] = routeKey
//       .split(" - ")
...
//       const [_, routePart, time] = match;
//       const [from, to] = routePart
//         .split(" - ")
//         .map((s) => s.trim().toLowerCase());
...
//       if (from === mainFrom && to === mainTo) {
//         currentTime = time.trim();
//         if (!finalOutput[routeKey][currentTime]) {
//           finalOutput[routeKey][currentTime] = {};
//         }
//       } else if (currentTime) {
//         finalOutput[routeKey][currentTime][cleanLabel] = records[label];
//       }
//     }
//   }
  ...
  try {
    ...
    for (let i = 0; i < serviceCards.length; i++) {
      ...
      const srpRating = await card
        .$eval("div[class*='ServiceWise---srpTrend']", (el) => {
          const match = el.textContent.trim().match(/SRP Rank\s*(\d+)/i);
          return match ? match[1] : "N/A";
        })
        .catch(() => "N/A");

      if (!response[routeKey][mainBusTime]) {
        response[routeKey][mainBusTime] = {};
      }

      const mainRouteName = `${route.redBusSource} - ${route.redBusDestination}`;
      const fullLabel = `${mainRouteName} - ${mainBusTime}`;
      response[routeKey][mainBusTime][fullLabel] = {
        rating: mainRouteRating,
        srpRating: srpRating,
      };
        ...
        for (let j = 0; j < viaRoutes.length; j++) {
          ...
          try {
            ...
            const viaSrpRating = await via
              .$eval("div[class*='ServiceWise---srpTrend']", (el) => {
                const match = el.textContent.trim().match(/SRP Rank\s*(\d+)/i);
                return match ? match[1] : srpRating;
              })
              .catch(() => srpRating); // fallback to main card srpRating
            ...
            if (
              ...
              const fullLabel = `${cleanRouteName} - ${normalizeTime(
                viaArrivalTime
              )}`;

              const isMainRoute =
                cleanRouteName.toLowerCase() ===
                `${route.redBusSource
                  .trim()
                  .toLowerCase()} - ${route.redBusDestination
                  .trim()
                  .toLowerCase()}`;

              if (!isMainRoute) {
                const normalizedViaTime = normalizeTime(viaArrivalTime);
                const fullLabel = `${cleanRouteName} - ${normalizedViaTime}`;

                if (!response[routeKey][mainBusTime][fullLabel]) {
                  response[routeKey][mainBusTime][fullLabel] = {
                    rating: viaRating,
