...
app.post("/scrapper/playwright", async (req, res) => {
  try {
    ...
    //   },
      ...
      //   "SR No": 1341,
      //   RouteName:
      //     "Shirpur To Pune Via Dhule 2x1(36) AC Sleeper, Prasanna - Purple Bus",
      //   Time: "07:30 PM",
      //   "From City": "Shirpur",
      //   "To City": "Nardana",
      ...
      const srpRating = await card
        .$eval(
          ...
          (el) => {
            ...
            const match = text.match(/SRP\s*(?:Rank)?\s*(\d+)/i);
      ...
      if (viaRoutesBtn) {
        ...
        for (let j = 0; j < viaRoutes.length; j++) {
          ...
          try {
            ...
            const [fromCity, toCity] =
              cleaned?.split(/\s*-\s*/).map((s) => s.trim()) || [];
            const subArrivalTime = normalizeTime(viaArrivalTime);
            const uniqueKey = `${fromCity}|${toCity}|${subArrivalTime}`;
            if (seenViaKeys.has(uniqueKey)) {
              console.log(`🔁 Skipping duplicate via segment: ${uniqueKey}`);
              continue;
            }
            seenViaKeys.add(uniqueKey);
            ...
            if (trendBtn) {
              try {
                ...
                console.log(
                  ` Error extracting full SRP for Via Route [${j + 1}]:`,
                  e.message
                );
              }
            } else {
            ...
            const match = await findMatchingSchedule(
              subArrivalTime,
              mongoDocs,
              fromCity,
              toCity
            ...
            console.log(` Error processing via route ${j + 1}: ${e.message}`);
          }
        }
      }
    }
