...
base_url = 'https://dev.aiqod.com:843'
app = Flask(__name__)
cors = CORS(app)
# myclient = pymongo.MongoClient('mongodb://domo-1:domonodel@127.0.0.1:27017/demo')
myclient = pymongo.MongoClient('mongodb://dev-SU:mdhdiIQTS@172.168.1.199:27017/aiqod-dev')
db = myclient['aiqod-dev']
...
def get_ocr(ImagesPath_Array,fileReferenceNumber,documentType,userId,subscriberId,orgId,url):
    ...
    r = requests.post(f'{url}/gibots-ocr/tesseract_ocr_wbw_new', data=json.dumps(payload), headers={'Content-Type': 'application/json'})
    return r.json()
...
def prediction(mlInput,runTrainingForInvoice,referenceNumber,templateName,userId,subscriberId,orgId,url):
    payload = {
        "input": {
            ...
            "configValue": "{'anchor_only':['Vendor_Name','Vendor_GSTIN'],'db':'aiqod-dev','dbConnection':'mongodb://dev-SU:mdhdiIQTS@172.168.1.199:27017/?authSource=aiqod-dev','requestURL':'http://localhost:8992/gibots-orch/orchestrator/botsiowrite','acknowledgeURL':'http://localhost:8992/gibots-orch/orchestrator/acknowledge', 'mountpath':'/home/server/scanning/'}",
            "userId": userId,
            "subscriberId": subscriberId,
            "orgId": orgId,
            "return": True,
...
def extraction_process_steps(filePath,ref_no,doc_type,template_name,userId,orgId,subscriberId):
    ...
    print("Starting convertPdfToPng for file:", filePath)
    # output = convertPdfToPng(filePath, userId, subscriberId, orgId,'http://172.168.1.199:2004')
    output = convertPdfToPng(filePath, userId, subscriberId, orgId,'http://localhost:8991')
    pdf_end = time.monotonic()
    print(f"convertPdfToPng took: {pdf_end - pdf_start:.4f} seconds")
    print('convertPdfToPng output:', output)
    ...
    output = get_ocr(output_image_array, ref_no, doc_type, userId, subscriberId, orgId,'http://localhost:2222')
    ...
    output = preMLOps(ocr_output, ref_no, userId, subscriberId, orgId,'http://localhost:8990')
    premlops_end = time.monotonic()
    print(f"preMLOps took: {premlops_end - premlops_start:.4f} seconds")
    print("preMLOps output:", output)
    ...
    return output, len(output_image_array)
...
# Format the datetime object
        ...
        response = requests.post(
            f"http://localhost:8990/adhigam-api/mapper/scan/multi",
            # f"http://172.168.1.199:2001/adhigam-api/mapper/scan/multi",
        ...
        for data in response_data['data']:
            ...
            res,pg_count = extraction_process_steps(org_file_path,ref_no,doc_type,pdf_client_name,user_id,orgId,sub_id)
            result = res['data']
