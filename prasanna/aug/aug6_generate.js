...
function convertExcel(jsonData, reviewsToInsert,excelForError) {
  const timestamp = Date.now();
  const formattedDate = new Date()
    .toLocaleDateString("en-GB")
    .replaceAll("/", "-");
  ...
  // Convert to Excel // excelForError
  const ws = XLSX.utils.json_to_sheet(rows);
  ...
  XLSX.utils.book_append_sheet(wb, errors, "Exceptions");
...
app.post("/scrapper/playwright", async (req, res) => {
  try {
    ...
    for (const route of mongoDocsRedBus) {
      ...
      try {
        let redbusRes = await searchRedBus(route, response, mongoDocs);
        if(!redbusRes){
        ...
        await searchRedBus(route, response, mongoDocs);
         const key = `${route.redBusSource} → ${route.redBusDestination}`;

        if (!errors[key]) {
          errors[key] = [];
        }

        errors[key].push(`[Redbus][Outer Error] ${e.message}`);
      }
    }
    ...
    let convertExceal = convertExcel(response, reviewsToInsert,excelForError);
