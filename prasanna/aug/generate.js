...
function convertExcel(
      ...
      if (!/AM|PM/i.test(key)) {
        ...
        for (const subTime in timeBlocks[subroute]) {
          ...
          staticRatings[`${subroute}-${subTime}`] = {
            ...
            RouteID: entry?.RouteID ?? "NA", // ✅ Added srpRating
            RouteTimeID: entry?.RouteTimeID ?? "NA", // ✅ Added srpRating
          };
        }
      }
    }
      ...
      for (const subroute in subroutes) {
        ...
        for (const subTime in times) {
          ...
          rows.push({
            ...
            "Paytm Rating": entry?.paytmBus ?? staticEntry.paytmBus ?? "NA",
            "Abhibus Rating": entry?.abhibus ?? staticEntry.abhibus ?? "NA",
            "Redbus Rating": entry?.redbus ?? staticEntry.redbus ?? "NA",
            "Redbus Srp Ranking":
              entry?.srpRating ?? staticEntry.srpRating ?? "NA",
            RouteID: entry?.RouteID ?? staticEntry.RouteID ?? "NA",
            RouteTimeID: entry?.RouteTimeID ?? staticEntry.RouteTimeID ?? "NA",
          });
        }
      }
    }
  ...
  let merged = rows.map((aItem) => {
    const bMatch = yesterdayDriverDetails.find(
      (bItem) =>
        bItem.RouteID === aItem.RouteID &&
        bItem.RouteTimeID === aItem.RouteTimeID
    );

    // if (!bMatch) {
    //   console.warn(
    //     `No driver details match for RouteID: ${aItem.RouteID}, RouteTimeID: ${aItem.RouteTimeID}`
    //   );
    // } else if (!bMatch["PickupMan Phone"]) {
    //   console.warn(
    //     `PickupMan Phone missing for RouteID: ${aItem.RouteID}, RouteTimeID: ${aItem.RouteTimeID}`
    //   );
    // }

    fieldsToCopy.forEach((field) => {
      aItem[field] = bMatch ? bMatch[field] ?? "" : "";
    });

    return aItem;
  });

  merged = merged.map(({ RouteID, RouteTimeID, ...rest }) => {
  ...
  //   // Split Driver Phone if it has 20+ digits
  //   if (item["Driver Phone"] && /^\d{20}$/.test(item["Driver Phone"])) {
  //     const phone1 = item["Driver Phone"].slice(0, 10);
  //     const phone2 = item["Driver Phone"].slice(10, 20);
  //     item["Driver Phone 1"] = phone1;
  //     item["Driver Phone 2"] = phone2;
  //     delete item["Driver Phone"];
  //   }
  //   return item;
  // });
  ...
  merged = merged.map((item) => {
    if (item["Driver Phone"]) {
      const phone = item["Driver Phone"].replace(/\D/g, ""); // remove non-digits
      if (phone.length === 20) {
        item["Driver Phone 1"] = phone.slice(0, 10);
        item["Driver Phone 2"] = phone.slice(10, 20);
      } else if (phone.length === 10) {
        item["Driver Phone 1"] = phone;
      }
    }
    delete item["Driver Phone"]; // <-- always delete it
    return item;
  });
  ...
  return { jsonData: merged, publicUrl };
}
  ...
  try {
    ...
    let RedbusRoute = [
      {
        "SR No": 1326,
        RouteName:
          "Mumbai To Pune via Amravati 2x1(36) AC Sleeper, Prasanna - Purple Bus",
        Time: "09:15 PM",
        "From City": "Amravati",
        "To City": "Mumbai",
    ...
    function extractFromToFromRouteName(routeName) {
      const match = routeName.match(
        /^(.+?)\s+To\s+([A-Za-z\s\-]+?)(?=\s*(?:Via|Bharat|Benz|\d+x\d+|\d+\*\d+|\(\d+\)|AC|A\/c|Non|Sleeper|Seater|,|$))/i
      );
    ...
    for (const route of mongoDocsRedBus) {
      ...
      try {
        ...
        await searchRedBus(route, response, mongoDocs, errors);
      }
    }
    ...
    try {
      ...
      //-----------------------------------------------------------------------------------------------
      // function normalizeBusNumber(busNum) {
      //   if (typeof busNum !== "string") return "";
      //   return busNum
      //     .toUpperCase()
      //     .replace(/^([A-Z]{2})(\d{2})([A-Z]{1,2})(\d{4})$/, "$1-$2-$3-$4");
      // }
      // const dateAndVehicleConditions = reviewsToInsert
      //   .map((r) => {
      //     const d = new Date(r.journeyDate);
      //     const start = new Date(d.getFullYear(), d.getMonth(), d.getDate());
      //     const end = new Date(d.getFullYear(), d.getMonth(), d.getDate() + 1);
      //     const normalizedVehicle = normalizeBusNumber(r.vehicle);
      //     if (!normalizedVehicle) {
      //       console.warn(
      //         "Skipping review with invalid vehicle format:",
      //         r.vehicle
      //       );
      //       return null;
      //     }
      ...
      //   )
      //   .toArray();
      // console.log("Fetched driver details:", scheduleDocs);
      // reviewsToInsert = reviewsToInsert.map((review) => {
      //   const d = new Date(review.Date);
      //   const start = new Date(d.getFullYear(), d.getMonth(), d.getDate());
      //   const end = new Date(d.getFullYear(), d.getMonth(), d.getDate() + 1);
      //   const matchingSchedule = scheduleDocs.find(
      //     (s) =>
      //       s["Bus Number"] === review.vehicle &&
      //       new Date(s["Schedule Date"]) >= start &&
      //       new Date(s["Schedule Date"]) < end
      //   );
      //-------------------------------------------------------------------------------------------------------------
      //uncomm
      ...
      const dateAndVehicleConditions = reviewsToInsert
        .map((r) => {
          if (!r.vehicle) {
            console.warn("Skipping review with missing vehicle:", r);
            return null;
          }
          const d = new Date(r.journeyDate);
          if (isNaN(d.getTime())) {
            console.warn(
              "Skipping review with invalid journeyDate:",
              r.journeyDate
            );
            return null;
          }
      ...
      const cleanedScheduleDocs = scheduleDocs.map((item) => {
        if (item["Driver Phone"]) {
          const phone = item["Driver Phone"].replace(/\D/g, ""); // remove non-digits
          if (phone.length === 20) {
            item["Driver Phone 1"] = phone.slice(0, 10);
        ...
        delete item["Driver Phone"]; // remove original
        return item;
      });
      console.log("Fetched driver details:", cleanedScheduleDocs);
      ...
      reviewsToInsert = reviewsToInsert.map((review) => {
        ...
        const normalizedVehicle = normalizeBusNumber(review.vehicle);

        const matchingSchedule = scheduleDocs
          .filter(
            (s) =>
              s["Bus Number"] === normalizedVehicle &&
              new Date(s["Schedule Date"]) >= start &&
              new Date(s["Schedule Date"]) < end
          )
          .sort(
        ...
        // const journeyDateObj = new Date(review.journeyDate);
        // const isValidDate = !isNaN(journeyDateObj);
        const {
          "Schedule Date": _scheduleDate,
          "Bus Number": _busNumber,
          ...scheduleWithoutDates
        } = matchingSchedule || {};
        ...
        return {
          ...
          vehicle:
            review.vehicle &&
            typeof review.vehicle == "string" &&
            /^[A-Za-z]{2}\d{2}[A-Za-z]{2}\d{4}$/.test(review.vehicle)
              ? review.vehicle
                  .toUpperCase()
                  .replace(
                    /^([A-Z]{2})(\d{2})([A-Z]{2})(\d{4})$/,
              ...
              : review.vehicle,
          Date:
            review.journeyDate && review.journeyDate instanceof Date
              ? review.journeyDate.toISOString().split("T")[0]
              : null,
          Time:
            review.journeyDate && review.journeyDate instanceof Date
              ? review.journeyDate.toISOString().split("T")[1].split(".")[0]
              : null,
          createdAt: new Date(),
        };
      });
      ...
      //   .find({})
      //   .toArray();
      // For each review, create a matching condition
      //   reviewsToInsert = reviewsToInsert.map((review) => {
      //     return{
      //         const driverDetails =
      //     }
      ...
      let dbreviewsToInsert = reviewsToInsert.map((review) => {
        ...
        const matchingSchedule = cleanedScheduleDocs
          .filter(
            (s) =>
              s["Bus Number"] === normalizedVehicle &&
              new Date(s["Schedule Date"]) >= start &&
        ...
        return {
          ...
          description: `Mobile : ${review.Mobile},\n
              ...
              Journey Date: ${review.Date},\n
              Journey Time: ${review.Time},\n
    ...
    let yesterdayDriverDetails = await getYesterdayDriverDetails(db);
    let excelForError = convertErrorsToExcel(errors);
    let convertExceal = convertExcel(
      response,
      reviewsToInsert,
      excelForError,
      yesterdayDriverDetails
...
async function getYesterdayDriverDetails(db) {
  try {
    const collection = db.collection("ScheduleMaster_driver");
    ...
    console.log("Yesterday's records:", docs.length);
    return docs;
  } catch (e) {
    console.error("Error in getYesterdayDriverDetails: ", e);
    return [];
  }
}
...
async function searchPaytm(route, response, mongoDocs, errors) {
  ...
  try {
    ...
    for (let i = 0; i < buses.length; i++) {
      ...
      if (!match) {
        errors[routeKey].push(
          `[Paytm] Not Matching time: ${subArrivalTime} with Bus ScheduleMaster`
        );
        console.log(
  ...
  try {
    ...
    for (const bus of buses) {
      ...
      if (!match) {
        ...
        errors[routeKey].push(
          `[Abhibus] Not Matching time: ${subArrivalTime} with Bus ScheduleMaster`
        );
...
async function searchRedBus(route, response, mongoDocs, errors) {
  ...
  try {
    ...
    for (let i = 0; i < serviceCards.length; i++) {
      ...
      if (!match) {
        console.log(
          `--No matching schedule found for main route: ${fromCity} → ${toCity} @ ${viaArrivalTime}`
        );
        // errors[routeKey].push(
        errors[routeKey].push(
          `[Redbus] Not Matching time: ${viaArrivalTime} with Bus ScheduleMaster for ${fromCity} → ${toCity} Subroute`
        );
        continue;
      }
      console.log(
